 ...
 */
class WC_Freterapido_Shipping_Simulator extends WC_Freterapido {
	...
	public function scritps() {
		if ( is_product() ) {
			wp_enqueue_style( 'shipping-simulator', plugins_url( 'assets/css/simulator.css', plugin_dir_path( __FILE__ ) ), array(), WC_Freterapido_Main::VERSION, 'all' );
			wp_enqueue_script( 'shipping-simulator', plugins_url( 'assets/js/simulator.js', plugin_dir_path( __FILE__ ) ), array( 'jquery' ), WC_Freterapido_Main::VERSION, true );
			wp_localize_script(
				'shipping-simulator',
				'shipping_simulator',
				array(
	...
	public static function simulator() {
		...
		if ( $product->is_in_stock() && in_array( $product->product_type, array( 'simple', 'variable' ) ) ) {
			wc_get_template(
				'single-product/shipping-simulator.php', array(
					'product' => $product,
					'style'   => $style,
					'ids'     => $ids,
				), '', WC_Freterapido_Main::get_templates_path()
			);
	...
	public function ajax_simulator() {
		$freterapido = new WC_Freterapido( $_POST['instance_id'] );
		...
		$shipping_values = $freterapido->fr_calculate( $package );
...
new WC_Freterapido_Shipping_Simulator();
